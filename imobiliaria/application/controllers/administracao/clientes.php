...
class Clientes extends CI_Controller {
    ...
    public function salvar_inclusao() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($retorno) {
                ...
                $this->session->set_flashdata("erro", "Houve um erro ao cadastrar o Cliente.");
            }
    ...
    public function salvar_alteracao() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['cpf'] = $this->input->post('cpf');
            $this->db->where('id_cliente', $id);
            $retorno = $this->db->update('clientes', $data);
            if ($retorno) {
                $this->session->set_flashdata("sucesso", "Cliente atualizado com sucesso.");
            } else {
                $this->session->set_flashdata("erro", "Houve um erro ao atualizar o Cliente.");
            }
            redirect(base_url() . "administracao/clientes/1");
        }
    }
    ...
    function excluir($id) {
        $this->db->where('id_cliente', $id);
        $retorno = $this->db->delete('clientes');
        if ($retorno) {
            $this->session->set_flashdata("sucesso", "Cliente excluído com sucesso.");
        } else {
            $this->session->set_flashdata("erro", "Houve um erro ao excluir o Cliente.");
        }
        redirect(base_url() . "administracao/clientes/1");
    }

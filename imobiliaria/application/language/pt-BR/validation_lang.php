<?php

$lang['required'] = 'O campo %s é obrigatório.';
$lang['is_unique']= "O campo %s deve conter um valor único.";
$lang['isset'] = 'O campo %s deve ter algum valor.';
$lang['valid_email'] = 'O campo %s deve conter um endereço de e-mail válido.';
$lang['valid_emails'] = 'O campo %s deve conter todos os endereços de e-mail válidos.';
$lang['valid_url'] = 'O campo %s deve conter uma URL válida';
$lang['valid_ip'] = 'O campo %s deve conter um endereço IP válido.';
$lang['min_length'] = 'O campo %s deve ter pelo menos %s caracteres em comprimento.';
$lang['max_length'] = 'O campo %s não pode exceder a quantidade de %s caracteres.';
$lang['exact_length'] = 'O campo %s deve ter exatamente %s caracteres em comprimento.';
$lang['alpha'] = 'O campo %s deve conter somente caraceteres do alfabeto.';
$lang['alpha_numeric'] = 'O campo %s deve conter somente caracteres alfa-numéricos.';
$lang['alpha_dash'] = 'O campo %s deve conter somente caracteres alfa-numéricos, sublinhados, e hífens.';
$lang['numeric'] = 'O campo %s deve conter um número.';
$lang['integer'] = 'O campo %s deve conter um inteiro.';
$lang['matches'] = 'O campo %s não corresponde a um campo %s.';
?>